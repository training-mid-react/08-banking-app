...
export const depositFromExternal = async (amount: number, accountNumber: string, customer: string, destinationAccount: string, token: string): Promise<any> => {
  ...
  const [data, error] = await handleTryCatch(http<DepositRequest, DepositResponse>({
    ...
    url: urlResources.depositFromExternal(),
    data: depositRequest,
    token: token,
  }));
