...
export const depositFromATM = async (amount: number, accountNumber: string, customer: string, destinationAccount: string, token: string): Promise<any> => {
  ...
  const [data, error] = await handleTryCatch(http<DepositRequest, DepositResponse>({
    ...
    url: urlResources.depositFromATM(),
    data: depositRequest,
    token: token,
  }));
