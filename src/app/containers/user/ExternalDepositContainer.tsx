...
export default function ExternalDepositContainer() {
  const { accountNumber, ExternalDepositFunction } = useUser();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await ExternalDepositFunction(amount, destinationAccount);
    } catch (error: any) {
  ...
  return (
    <LayoutBase title="Depósito desde otra cuenta">
      <DepositForm 
        originAccount={accountNumber} 
